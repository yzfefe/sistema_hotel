...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['buscar_hospede'])) {
        ...
        if ($stmt->execute()) {
            ...
            if ($result->num_rows > 0) {
                ...
                // Buscar reservas de salão do hóspede
                $sql_reservas = "SELECT r.*, s.nome as nome_salao, s.preco, d.nome as nome_decoracao
                    FROM reservas_salao r
                    JOIN saloes s ON r.id_salao = s.id_salao
                    LEFT JOIN decoracoes d ON r.id_decoracao = d.id_decoracao
                    WHERE r.id_hos = ? AND r.status = 'Pendente'";
    ...
    if (isset($_POST['gerar_conta'])) {
        ...
        $sql_reserva = "SELECT data_inicio, id_salao, data_criacao FROM reservas_salao WHERE id_reserva = ?";
        $stmt = $conn->prepare($sql_reserva);
        $stmt->bind_param("i", $id_reserva);
        $stmt->execute();
        $reserva = $stmt->get_result()->fetch_assoc();
        
        // Obter preço do salão PRIMEIRO
        $sql_salao = "SELECT preco FROM saloes WHERE id_salao = ?";
        $stmt = $conn->prepare($sql_salao);
        $stmt->bind_param("i", $reserva['id_salao']);
        $stmt->execute();
        $preco = $stmt->get_result()->fetch_assoc()['preco'];
        
        // Calcular dias de uso usando a data atual como data_fim
        $data_inicio = new DateTime($reserva['data_inicio']);
        $data_fim = new DateTime(); // Data atual
        $intervalo = $data_inicio->diff($data_fim);
        $dias = $intervalo-> d;

        if($dias>=1){
            $dias_totais = $dias * $preco;

            $sql_atualiza_reserva = "UPDATE reservas_salao SET 
                            data_fim = NOW(), 
                            valor_total = ?
                            WHERE id_reserva = ?";
            $stmt = $conn->prepare($sql_atualiza_reserva);
            $stmt->bind_param("di", $total_conta, $id_reserva);
            $stmt->execute();
        }else{
            $total_conta = $preco; // Calcula os gastos totais
        
            // Atualizar a reserva com a data final atual e o valor calculado
            $sql_atualiza_reserva = "UPDATE reservas_salao SET 
                                    data_fim = NOW(), 
                                    valor_total = ?
                                    WHERE id_reserva = ?";
        ...
        $conta_gerada = [
            ...
            'data_fim' => $data_fim->format('Y-m-d H:i'),
            'id_salao' => $reserva['id_salao']
        ];
    ...
    if (isset($_POST['processar_pagamento'])) {
        ...
        $stmt->bind_param("di", $total_conta, $id_hos);
        $stmt->execute();
        
        // Encerrar reserva
        $sql_encerrar = "UPDATE reservas_salao SET 
                         status = 'Concluída'
                         WHERE id_reserva = ?";
        ...
        // Liberar salão
        $sql_salao_update = "UPDATE saloes SET disponibilidade = 'Disponível' WHERE id_salao = ?";
        $stmt = $conn->prepare($sql_salao_update);
...
<head>
    ...
    <title>Fechar Estadia de Salão - Hotel</title>
...
<body>
    ...
    <div class="container container-main">
        ...
        <h1 class="header-title">
            <i class="bi bi-calendar-event"></i> Fechar Estadia de Salão
        </h1>
        <p class="text-center text-muted mb-4">Gerenciamento de contas de salões de eventos</p>
        ...
        <?php if ($hospede): ?>
            ...
            <!-- Reservas de Salão Ativas -->
            <?php if (!empty($reservas)): ?>
                <div class="row">
                    <div class="col-12">
                        <h4 class="mb-3 text-secondary"><i class="bi bi-calendar-event"></i> Reservas de Salão Ativas</h4>
                        
                        <?php foreach ($reservas as $reserva): ?>
                            <div class="card card-reserva">
                                <div class="card-header d-flex justify-content-between align-items-center">
                                    <h5 class="mb-0 text-white">
                                        <i class="bi bi-building"></i> <?= htmlspecialchars($reserva['nome_salao']) ?>
                                    </h5>
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-4">
                                            <p><i class="bi bi-calendar-check text-primary"></i> <strong>Data Início:</strong> 
                                            <?= date('d/m/Y', strtotime($reserva['data_criacao'])) ?></p>
                                            <p><i class="bi bi-calendar-x text-primary"></i> <strong>Data Fim:</strong>
                                                <?php if (!empty($reserva['data_fim'])): ?>
                                                    <?= date('d/m/Y', strtotime($reserva['data_fim'])) ?>
                                                <?php elseif ($conta_gerada && $conta_gerada['id_reserva'] == $reserva['id_reserva']): ?>
                                                    <?= date('d/m/Y', strtotime($conta_gerada['data_fim'])) ?>
                                                <?php endif; ?>
                                            </p>
                                        </div>
                                        <div class="col-md-4">
                                            <?php if ($reserva['nome_decoracao']): ?>
                                                <p><i class="bi bi-flower1 text-primary"></i> <strong>Decoração:</strong> <?= htmlspecialchars($reserva['nome_decoracao']) ?></p>
                                            <?php endif; ?>
                                            <form method="post">
            ...
            <?php else: ?>
                <div class="alert alert-info">
                    <i class="bi bi-info-circle"></i> Nenhuma reserva de salão ativa encontrada para este hóspede.
                </div>
            <?php endif; ?>
        <?php endif; ?>
    </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <ul class="list-group">
                                <li class="list-group-item d-flex justify-content-between">
                                    ...
                                    <span><?= date('d/m/Y H:i', strtotime($conta_gerada['data_criacao'])) ?> a <?= date('d/m/Y H:i', strtotime($conta_gerada['data_fim'])) ?></span>
                                </li>
                                <li class="list-group-item d-flex justify-content-between">
                                    <span><i class="bi bi-clock text-primary"></i> Dias de Uso:</span>
                                    <span><?= $conta_gerada['dias'] ?></span>
                                </li>
                                <li class="list-group-item d-flex justify-content-between">
                                    <span><i class="bi bi-cash-coin text-primary"></i> Valor por cada dia:</span>
                                    <span>R$ <?= number_format($conta_gerada['preco'], 2, ',', '.') ?></span>
                                </li>
                        ...
                        <div class="col-md-6">
                            ...
                            <div id="paymentDetails" class="mt-3" style="display: none;">
                                ...
                                <form id="paymentForm" method="post">
                                    ...
                                    <input type="hidden" name="id_salao" value="<?= $conta_gerada['id_salao'] ?>">
