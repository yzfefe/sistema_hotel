...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['gerar_conta'])) {
        ...
        // Verificar se havia promoção durante o período de hospedagem
        $sql_preco = "SELECT 
                        IFNULL(
                            (SELECT preco_promocional 
                            FROM promocoes_quartos 
                            WHERE id_quarto = ? 
        ...
        $stmt = $conn->prepare($sql_preco);
        $stmt->bind_param("isi", $reserva['id_quarto'], $reserva['data_reserva'], $reserva['id_quarto']);
        $stmt->execute();
        $preco_diaria = $stmt->get_result()->fetch_assoc()['preco_final'];
        ...
        $conta_gerada = [
            ...
            'data_fim' => date('Y-m-d'),
            'id_quarto' => $reserva['id_quarto'] // Adicionado para uso no processamento
        ];
    }
